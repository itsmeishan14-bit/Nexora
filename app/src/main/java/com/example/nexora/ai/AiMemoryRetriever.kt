...
class AiMemoryRetriever(
    ...
    private fun calculateRelevance(memory: AiMemoryItem, request: AiRequest): Int {
        ...
        score += when (request.type) {
            AiRequestType.NEXT_TASK -> if (memory.category == AiMemoryCategory.TASK_PATTERN || memory.category == AiMemoryCategory.PRODUCTIVITY_PATTERN || memory.category == AiMemoryCategory.TASK_SIZE_PATTERN) 25 else 0
            AiRequestType.DAILY_PLAN -> if (memory.category == AiMemoryCategory.PLANNING_PATTERN || memory.category == AiMemoryCategory.WORKLOAD_PATTERN || memory.category == AiMemoryCategory.TASK_SIZE_PATTERN) 30 else 0
            AiRequestType.GOAL_ANALYSIS -> if (memory.category == AiMemoryCategory.GOAL_PATTERN) 35 else 0
            AiRequestType.PRODUCTIVITY_ANALYSIS -> if (memory.category == AiMemoryCategory.PRODUCTIVITY_PATTERN || memory.category == AiMemoryCategory.WORKLOAD_PATTERN) 40 else 0
            AiRequestType.CHAT -> if (memory.importance == AiMemoryImportance.HIGH) 20 else 10
            else -> 0
        }
        ...
        score += when (memory.confidence) {
            ...
            AiMemoryConfidence.HIGH -> 15
            AiMemoryConfidence.MEDIUM -> 10
            AiMemoryConfidence.LOW -> 5
            else -> 0
        }
